...
function searchFlights() {
  ...
  const ffrom = urlParams.get("from");
  const tto = urlParams.get("to");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const getParam = (key) => urlParams.get(key) || 'N/A';
  ...
  for (const [param, id] of Object.entries(dataMap)) {
    const element = document.getElementById(id);
    if (element) element.textContent = getParam(param);
  }

  const seats = getArrayParam("seats");
  const seatsElement = document.getElementById("seats");
  if (seatsElement) seatsElement.textContent = seats.length ? seats.join(", ") : "None";

const pricePerPassenger = parseFloat(getParam("price")) || 0;
const adults = parseInt(getParam("adults")) || 0;
const children = parseInt(getParam("children")) || 0;
const seniors = parseInt(getParam("seniors")) || 0;
const infants = parseInt(getParam("infants")) || 0;

const totalPassengers = adults + children + seniors + infants;
const totalPrice = pricePerPassenger * totalPassengers;
...
if (priceElement) priceElement.textContent = `₹${totalPrice}`;
  ...
  const mealSummary = [];
const vegCount = parseInt(getParam("veg")) || 0;
const nonVegCount = parseInt(getParam("nonVeg")) || 0;
const specialCount = parseInt(getParam("special")) || 0;

if (vegCount) mealSummary.push(`${vegCount} Veg`);
if (nonVegCount) mealSummary.push(`${nonVegCount} Non-Veg`);
if (specialCount) mealSummary.push(`${specialCount} Special`);

const mealElement = document.getElementById("meals");
if (mealElement) mealElement.textContent = mealSummary.length ? mealSummary.join(", ") : "None";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  cancelButton.addEventListener("click", function () {
    const confirmed = confirm("Are you sure you want to cancel the flight booking?");
    if (confirmed) {
      if (flightInfoDisplay) {
        flightInfoDisplay.style.display = "none";
      }
    }
  });
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const overlay = document.getElementById("changeModalOverlay");
  const modal = document.getElementById("changeModal");

  function isTooLateToChange(departureDateStr) {
    ...
    const diffDays = (departureDate - today) / (1000 * 60 * 60 * 24);
    return diffDays <= 0;
  }

  changeButton.addEventListener("click", function () {
    const departureDateText = document.getElementById("ddepartureDate").textContent.trim();

    if (!departureDateText || isTooLateToChange(departureDateText)) {
      alert("Change is not possible on or one day before the departure date.");
      return;
    }
...
document.addEventListener("DOMContentLoaded", function () {
  const vegInput = document.getElementById("vegMeal");
  const nonVegInput = document.getElementById("nonVegMeal");
  const specialInput = document.getElementById("specialMeal");

  function parseMealString(mealStr) {
    const result = {
      veg: 0,
      nonVeg: 0,
      special: 0,
    ...
    mealStr.split(",").forEach(part => {
      const [count, type] = part.trim().split(" ");
      if (type.toLowerCase().includes("veg") && !type.toLowerCase().includes("non")) {
        result.veg = parseInt(count);
      } else if (type.toLowerCase().includes("non-veg")) {
        result.nonVeg = parseInt(count);
      } else if (type.toLowerCase().includes("special")) {
        result.special = parseInt(count);
      }
    });
  ...
  function populateMealsFromText() {
    const mealText = document.getElementById("meals").textContent.trim();
    const mealData = parseMealString(mealText);

    vegInput.value = mealData.veg;
    nonVegInput.value = mealData.nonVeg;
    specialInput.value = mealData.special;
  }

  const changeButton = document.getElementById("change");
  changeButton.addEventListener("click", populateMealsFromText);

function updateMealTotal() {
  const veg = parseInt(vegInput.value) || 0;
  const nonVeg = parseInt(nonVegInput.value) || 0;
  const special = parseInt(specialInput.value) || 0;
  const totalMeals = veg + nonVeg + special;

  const adults = parseInt(document.getElementById("aadults").textContent || "0");
  const children = parseInt(document.getElementById("cchildren").textContent || "0");
  const seniors = parseInt(document.getElementById("sseniors").textContent || "0");
  const infants = parseInt(document.getElementById("iinfants").textContent || "0");

  const totalPassengers = adults + children + seniors + infants;

  if (totalMeals > totalPassengers) {
    alert(`You can only select up to ${totalPassengers} meals (equal to the number of passengers).`);
    document.activeElement.value = 0;
    updateMealTotal();
  }
}
window.updateMealTotal = updateMealTotal;
...
document.querySelector(".continue-btn").addEventListener("click", function () {
    ...
    const newMealText = `${veg} veg, ${nonVeg} non-veg, ${special} special`;
    document.getElementById("meals").textContent = newMealText;

    document.getElementById("changeModalOverlay").style.display = "none";
  });